...
def runBiCGStab(A,b,coef_list, config:ConfigParser):
    if b is None:
        b = np.ones((config.getint('Data', 'dim'),1 ))
    if coef_list is None:
        M_inv = None
    else:
        M_inv = prec.parShiftJacobi(A,config.getint('Data', 'dim'), coef_list)
    ...
    with Pool() as pool:
        ...
        for ii in range(config.getint('Learn', 'iteration_count')):
            ...
            for scale, pm in product(config.getfloatList('Learn', 'scale'), [1, -1]):
                ...
                new_k_list, flag_list = zip(*pool.starmap(runBiCGStab, [(A, b, temp_coef, config) for A in data]))
...
if __name__ == '__main__':
    ...
    # coef = list(range(1,10))

    # M_inv = prec.parShiftJacobi(training_data[0],config.getint('Data', 'dim'), coef)
    # print(M_inv)
    # dense = M_inv.toarray().copy()
    # print(dense)
    ...
    with open(f'testData/shift_Jacobi_laplace_{config.get("Learn", "method")}.txt', mode = 'a') as txt_file:
        ...
        with Pool() as pool:
            ...
            jacobi_k_list, jacobi_flag_list = zip(*pool.starmap(runBiCGStab, [(A, None, [0], config) for A in test_data]))
            txt_file.write(f'Jacobi: {util.statStr(jacobi_k_list)}, flag: {np.sum(jacobi_flag_list)} \n\t{jacobi_k_list}\n\n')
            # M_inv = prec.parShift(config.getint('Data', 'dim'), coef_list)
            final_k_list, final_flag_list = zip(*pool.starmap(runBiCGStab, [(A, None, coef_list, config) for A in test_data]))
    ...
    n1 ,_,_ = plt.hist([non_precond_k_list,final_k_list,jacobi_k_list],bins=40, alpha = 1, label=['Non', 'precond', 'jacobi'], color=['c', 'm', 'y'])
