...
class dist():
    ...
    def rand(self, size= None):
        if self.dist_type in  ['uni', 'uniform', 'u']:
            return self.rng.uniform(self.param[0]-self.param[1],self.param[0]+ self.param[1], size = size)
        elif self.dist_type in ['normal', 'n']:
            return self.rng.normal(loc = self.param[0], scale = self.param[1], size = size)


def gen2dLaplace(config, seed = None):

    

    N = config.getint('Data', '1D_laplace_size')
    distribution = dist(param = config.getfloatList('Data', 'params'), rng = seed, dist_type = config.get('Data', 'dist_type'))
    ...
    return (spar.kron(I1, L) + spar.kron(L, I2)).tocsr()


def genLaplaceData(config, seed = None):
    ...
    for i in rng.integers(1e10, size = config.getint('Data', 'amount')):
        data.append(gen2dLaplace(config, seed=int(i)))
...
if __name__ == '__main__':
    ...
    laplace = gen2dLaplace(config).toarray()
    laplace[laplace == 0] = np.nan
    fig = plt.figure(figsize=(9,7))

    im1 = plt.imshow(laplace) 
    plt.colorbar(im1)
    plt.show()
