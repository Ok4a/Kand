...
if __name__ == '__main__':
    ...
    file_names = ['finalData/sub super par/diag_shift_jacobi_laplace_sign_par.txt']
    # file_names = ['SaveData/super_sub/diag_shift_jacobi_laplace_sign_1.txt']
    lines_list = []
    lines_list.append({'train': [29,223,463,731], 'test': [177,417, 685, 989]})
    ...
    axs.legend( title='Num par', ncol=2)
    fig.supylabel('Solver iteration')
    fig.supxlabel('Learning steps')
    fig.suptitle('Learning curves\n for different number of partitions')
    ...
    #     fig.legend(loc='center right', title='Num par')
    #     plt.ylabel('Solver iteration')
    #     plt.xlabel('Precond type')
    ...
    #         plt.hist([non,with_,jacobi],bins=40, alpha = 1, label=dens_label, color=c[:3])

    #         plt.boxplot((non, with_, jacobi), orientation='horizontal', tick_labels = dens_label,showmeans=False, notch=True, positions = [5,8, 11], widths=1.5)
...
#         plt.title(label=f'({data[seed][line_type]['stat'][coef]['iter2'][num]},{data[seed][line_type]['stat'][coef]['iter2'][num]})')
...
#     ani = animation.FuncAnimation(fig, update_hist, num_frame, fargs=(all_lines,))
    ...
    #         labels.append(f'num par: {num_superNsub[coef]}')
