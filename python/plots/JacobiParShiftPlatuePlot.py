...
if __name__ == '__main__':
    # file_names = ['finalData/new precond (jacobi_shift)/shift_Jacobi_laplace_sign.txt','finalData/new precond (jacobi_shift)/shift_Jacobi_laplace_median.txt','finalData/new precond (jacobi_shift)/shift_Jacobi_laplace_mean.txt']
    file_names = ['finalData/new precond (jacobi_shift)/shift_Jacobi_laplace_sign.txt']

    lines_list = []
    lines_list.append({'train': [564,22],'test': [810,102]})
    ...
    for k in range(len(order)):
        for index in seeds:
            ...
            axs.plot(lines['iter2'],lines['mean'], label = f'Mean', linestyle='-', color=c[index])
            axs.plot(lines['iter2'], lines['median'], label = f'Median', linestyle='--', color=c[index])
    ...
    # fig.supxlabel('Learn iteration',y=0.07)
    ...
    for att, measure in data.items():
        ...
        # axs.set_title(f'Seed: {seed}')
        axs.set_axisbelow(True)
        axs.set_xticks(x+width, ['Non','SIGN: 500','SIGN: 100','Jacobi'])
        axs.legend()
