...
def runBiCGStab(A,b,M_inv, config:ConfigParser, stop_futher = True, other = None):
    ...
    try:
        ...
        if M_inv is None:
            ...
            M_inv = (sparse.eye(config.getint('Data', 'dim')) + M_inv).tocsr()
        elif config.get('Precondition', 'diag') == 'jacobi':
            M_inv = (prec.Jacobi(A) + M_inv).tocsr()
        # if M_inv is None:
        #     pass
        # elif other == 'jacobi':
        #     M_inv = prec.Jacobi(A)
...
if __name__ == '__main__':
    ...
    with open(f'testData/{config.get("Precondition", "type")}_{config.get("Precondition", "diag")}_laplace_{config.get("Learn", "method")}.txt', mode = 'a') as txt_file:
