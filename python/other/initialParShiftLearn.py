...
def laplaceDataML(data, file, config: ConfigParser, precond_class: prec.shiftPrecond):
    ...
    flag_count_limit = config.getint('Learn', 'flag_count_limit')
    ...
    for ii in range(config.getint('Learn', 'iteration_count')):
        ...
        found_better = False
        for scale, pm in product(config.getfloatList('Learn', 'change_scales'), [1]):
            count += 1
            print(prev_mean_k, ii, count, prev_index, best_index, end = '\r')


            M_inv = precond_class.makePrecond(scale = scale * precond_class.rng.choice([-1, 1]))
            ...
            flag_count = np.count_nonzero(flag_list)

            if flag_count <= flag_count_limit: # if all converged  
...
if __name__ == '__main__':
    ...
    the_matrix = mmread(f'matrixData/{matrix_name}.mtx.gz')

    precond_type = config.get('Precondition', 'type')


    print(np.shape(the_matrix))
    config.set('Data', 'dim',str(np.shape(the_matrix)[0]))
    print(config.get('Data','dim'))
    ...
    with open(f'testData/old_Data_{matrix_name}.txt', mode = 'a') as txt_file:
        ...
        txt_file.write(f'Last: {util.statStr(final_k_list)}, BvN: {sign_LvN[1]}, WvN: {sign_LvN[-1]}, BvJ: {sign_LvJ[1]}, WvJ: {sign_LvJ[-1]}, flag: {np.count_nonzero(final_flag_list)} \n\t{final_k_list}\n\n')
