...
def laplaceDataML(data, file, config: ConfigParser):
                ...
                if  config.get('Learn', 'method') == 'median':
                    if new_median <= prev_median_k:
                        ...
                        # if new_median <= best_median:
                            # improved = True
                    elif 1 > rng.uniform() * (new_median-prev_median_k+1) and config.getboolean('Learn', 'allow_disimprovement'):
                        found_better = True
                    ...
                    if new_mean <= prev_mean_k:
                        ...
                        # if new_mean <= best_mean:
                            # improved = True
                    elif 1 > rng.uniform()*(new_mean-prev_mean_k+1) and config.getboolean('Learn', 'allow_disimprovement'):
                        found_better = True
    ...
    return coef_list, best_coef_list
...
if __name__ == '__main__':
    ...
    with open(f'testData/{config.get('Precondition', 'type')}_laplace_{config.get("Learn", "method")}.txt', mode = 'a') as txt_file:
        ...
        coef_list, best_coef_list = laplaceDataML(training_data, file = txt_file, config = config)
